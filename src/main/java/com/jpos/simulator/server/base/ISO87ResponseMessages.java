...
public class ISO87ResponseMessages {
    ...
    protected static ISOMsg createReversalResponse(ISOMsg m, String responseCode) throws ISOException {
        return createResponse(m, responseCode);
    }

    protected static ISOMsg createFileActionResponse(ISOMsg m, String responseCode) throws ISOException {
        ISOMsg response = createResponse(m, responseCode);
        // Specifically echo file action fields if needed, but only once
        if (m.hasField(91))
